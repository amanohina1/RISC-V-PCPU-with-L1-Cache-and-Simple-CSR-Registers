...
*   **哈佛结构**: 指令内存与数据内存分离。
    ...
    *   **数据内存 (D-Mem)**: 基于 DRAM 模型实现，容量为 16 Bytes x 256，即4KB，多周期延时读写，与处理器通过一个L1缓存交互。
*   **L1 数据缓存 (D-Cache)**:
...
*   **测试程序:** **递归计算斐波那契数列第15项 (F(15))**。(或者第N项也可以，不能太大，否则会爆栈)
```
_start:
    li      sp, 4000
    jal     ra, main
...
**结论:** CPI为 **1.16** 是一个**比较出色**的指标。它证明了CPU的流水线设计（特别是前递和冒险检测单元）效率极高，成功地解决了绝大多数数据冒险，使CPU在绝大部分时间里都保持着每个周期完成一条指令的高速运转。

### 流水线停顿归因分析

CPU为了执行37,028条指令，付出了 `42,952 - 37,028 = 5,924` 个停顿周期的代价。通过自定义性能计数器（分支采纳计数器和内存停顿周期计数器），可以精确地剖析这些停顿的来源：

| 停顿来源 | 相关周期/事件数 | 占总停顿比例 | 根本原因 |
| :--- | :--- | :--- | :--- |
| **控制冒险** | **~5,407** 周期 | **~91.3%** | **分支/跳转指令** |
| **数据冒险** | **517** 周期 | **~8.7%** | **内存加载-使用和数据缓存的Miss惩罚** |

*   **控制冒险是主导因素:**
    我们测量到总共有 **5,922** 次被采纳的分支/跳转，而这导致的非内存停顿周期高达 **5,407** 个。这表明，**当前设计为每一次控制流的改变，付出了约0.91个周期的固定开销**。这主要是由于流水线在等待分支结果时产生的“气泡”所致。并且随着计算的斐波那契的目标项变大，CPI有增大的趋势。
    在另外一项纯循环计数的实验中，因为静态分支预测错误导致的惩罚导致CPI几乎趋近于2，进一步确定了动态分支预测的必要性。

*   **数据冒险是次要因素:**
    由缓存Miss导致的停顿和无法通过前递完全消除的内存访问停顿只有 **517** 个周期，在总停顿中占比很小。
    当修改内存为16组二路组相联时，我们运行递归程序求解Fib（32）。在计算到4496万条指令时，经过测量发现，过程中因为访问内存导致的流水线停顿的周期只有150个，这更加说明了合适的缓存对于减少CPI的重要性和有效性。
...
我现在已经初步实现了一个支持识别B类指令的2位饱和动态分支预测器，成功将循环计数场景下的分支惩罚降低至0，也就是说CPI近似于1.0。但目前尚未实现对J类指令的支持，因为在实现递归计算菲比纳契数列的时候，会发生奇怪的死锁现象。但受制于调试技术的不足，暂时无法解释这一现象的根源。

## 未来工作
*   [ ] 实现自动化测试，运用Spike，QEMU等工具进行进一步模拟。
*   [ ] 优化分支预测逻辑。
*   [ ] 添加 L1 指令缓存。
...
本项目在开发过程中解决了一系列经典的硬件设计挑战。其中一些关键问题的分析与解决过程被记录在了 [设计与调试日志 (DESIGN_INSIGHTS.md)](./DESIGN_INSIGHTS.md) 中。
